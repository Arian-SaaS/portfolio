...
export interface BlackHoleHeroSectionProps
   ...
   * value itself and cannot read `oklch()`, `color-mix()`, or `rgb()`.
   */
  hotColor?: string;
  /** Mid-disc. Hex or CSS custom property — see `hotColor`. */
  midColor?: string;
  /** Coolest gas, at the outer edge. Hex or CSS custom property — see `hotColor`. */
  coolColor?: string;
  /**
...
/**
 * Where the ramp lands if a token is missing or holds something the shader
 * cannot parse. A hero that renders in the wrong blue beats a hero that
 * renders black on black, so these are never allowed to fail through.
 */
const FALLBACK_HOT = "#eafcff";
const FALLBACK_MID = "#22d3ee";
const FALLBACK_COOL = "#164e8c";

/**
 * Hex to linear light, or null if the string is not hex. The shader works in
 * linear and gamma-encodes at the end.
 *
 * Returning null rather than NaN matters: a CSS token can perfectly well hold
 * `oklch(...)` or an empty string, and `parseInt` on either yields NaN, which
 * propagates silently all the way into the uniform and paints the disc black.
 */
 ...
 *
 * The lookup is cached because it runs inside the frame loop, and
 * `getComputedStyle` is a style read — doing three of them sixty times a
 * second, forever, to fetch values that change perhaps twice in a session is
 * the kind of thing that quietly costs a millisecond a frame. The cache is
 * dropped whenever the theme class on `<html>` changes, which is the only
 * thing that can move a token underneath us.
 */
function makeColorResolver(host: HTMLElement) {
  ...
  return {
    ...
    get(raw: string, fallback: string): [number, number, number] {
      ...
      const name = trimmed.startsWith("--")
        ...
        : trimmed.startsWith("var(")
          ? trimmed.slice(4, -1).split(",")[0].trim()
          : "";
      const value = name
        ? getComputedStyle(host).getPropertyValue(name)
        : trimmed;

      const rgb = hexToLinear(value) ?? hexToLinear(fallback)!;
      cache.set(raw, rgb);
      return rgb;
    },
  };
...
export function BlackHoleHeroSection({
  ...
  // A ref rather than state, because the frame loop reads these sixty times a
  // second and must not tear down and rebuild the GL context every time a dial
  // moves. Written in an effect rather than during render: React may render a
  // component and throw the result away, and a ref written during a discarded
  // render is a write that escaped into a live animation loop. Costs one frame
  // of latency when a prop changes, which is invisible against a 46-second
  // wind cycle.
  useEffect(() => {
    props.current = {
      distance, elevation, azimuth, orbitSpeed, roll, fov, diskInner, diskOuter,
  ...
  useEffect(() => {
    ...
    const colors = makeColorResolver(host);
    ...
    /**
     * Take the canvas out of the picture and leave the background and whatever
     * copy sits on it. A canvas whose context has died does not go quietly: it
     * paints white, or the browser's broken-image mark, straight over the
     * background — which is worse than showing nothing at all.
     */
    ...
    function render(t: number) {
      ...
      const hot = colors.get(C.hotColor, FALLBACK_HOT);
      const mid = colors.get(C.midColor, FALLBACK_MID);
      const cool = colors.get(C.coolColor, FALLBACK_COOL);
    ...
    // The palette is read from CSS custom properties and cached, so a theme
    // switch has to invalidate it. next-themes swaps a class on <html>, which
    // is the one event that can move a token underneath the cache.
  ...
  return (
    <div
      ref={hostRef}
      className={`relative isolate h-full w-full overflow-hidden bg-bh-void ${className}`}
      {...rest}
    >
